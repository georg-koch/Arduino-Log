...
void setup()
{
  ...
  Log.debug(F(CR "******************************************" CR)); // Info string with Newline
  Log.debug("***          Logging example                " CR);     // Info string in flash memory
  Log.debug(F("******************* "));
  Log.debug("*********************** " CR); // two info strings without newline
}
...
void loop()
{
  ...
  Log.debug("Log as Info with integer values : %d, %d" CR, intValue1, intValue2);
  Log.debug(F("Log as Info with hex values     : %x, %X" CR), intValue1, intValue1);
  Log.debug("Log as Info with hex values     : %x, %X" CR, intValue2, intValue2);
  Log.debug(F("Log as Info with binary values  : %b, %B" CR), intValue1, intValue1);
  Log.debug("Log as Info with binary values  : %b, %B" CR, intValue2, intValue2);
  Log.debug(F("Log as Info with long values    : %l, %l" CR), longValue1, longValue2);
  Log.debug("Log as Info with bool values    : %t, %T" CR, boolValue1, boolValue2);
  Log.debug(F("Log as Info with string value   : %s" CR), charArray);
  Log.debug("Log as Info with string value   : %s" CR, stringValue1.c_str());
  Log.debug(F("Log as Info with float value   : %F" CR), floatValue);
  Log.debug("Log as Info with float value   : %F" CR, floatValue);
  Log.debug(F("Log as Info with double value   : %D" CR), doubleValue);
  Log.debug("Log as Info with double value   : %D" CR, doubleValue);
  Log.debug(F("Log as Debug with mixed values  : %d, %d, %l, %l, %t, %T" CR), intValue1, intValue2,
